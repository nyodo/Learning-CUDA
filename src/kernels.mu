...
bool currentDeviceUses32LaneWarps() {
  int device = 0;
  ...
  if (cached_device != device) {
    ...
    RUNTIME_CHECK(musaGetDeviceProperties(&properties, device));
    cached_device = device;
    cached_warp_size = properties.warpSize;
  }
  return cached_warp_size == 32;
}
...
__device__ __forceinline__ float warpSum32(float value) {
...
template <typename T, int BlockSize>
__global__ void rmsNormWarpKernel(const T* __restrict__ input,
                                  const T* __restrict__ weight,
                                  T* __restrict__ output, size_t hidden_dim,
                                  float eps) {
  ...
  sum_squares = warpSum32(sum_squares);
  if (lane == 0) {
  ...
  if (warp == 0) {
    float block_sum = lane < BlockSize / 32 ? warp_sums[lane] : 0.0F;
    block_sum = warpSum32(block_sum);
    if (lane == 0) {
  ...
  for (size_t col = threadIdx.x; col < hidden_dim; col += BlockSize) {
    ...
    output[row_offset + col] =
        fromFloat<T>(value * inv_rms * toFloat(weight[col]));
  }
}
...
void launchRmsNorm(const T* input, const T* weight, T* output, size_t rows,
  ...
  const bool benefits_from_warp_path =
      sizeof(T) == sizeof(float)
          ? (hidden_dim <= 512 || rows >= 8)
          : (hidden_dim <= 256 || hidden_dim >= 768 || rows >= 128);
  if (!benefits_from_warp_path || !currentDeviceUses32LaneWarps()) {
    if (hidden_dim <= 128) {
  ...
  if (hidden_dim <= 32) {
    rmsNormWarpKernel<T, 32><<<blocks, 32>>>(input, weight, output,
                                             hidden_dim, eps);
  } else if (hidden_dim <= 64) {
    rmsNormWarpKernel<T, 64><<<blocks, 64>>>(input, weight, output,
                                             hidden_dim, eps);
  } else if (hidden_dim <= 128) {
    rmsNormWarpKernel<T, 128><<<blocks, 128>>>(input, weight, output,
                                               hidden_dim, eps);
  } else {
    rmsNormWarpKernel<T, 256><<<blocks, 256>>>(input, weight, output,
                                               hidden_dim, eps);
  }
}
...
__global__ void flashAttentionWarpOnlineKernel(
  ...
  enum {
    kWarpSize = 32,
    kWarpsPerBlock = kAttentionBlockSize / kWarpSize,
    kValuesPerLane = HeadDim == 0 ? 8 : (HeadDim + kWarpSize - 1) / kWarpSize
  };
  const int lane = threadIdx.x & (kWarpSize - 1);
  const int warp = threadIdx.x / kWarpSize;
  const size_t query_row =
      static_cast<size_t>(blockIdx.x) * kWarpsPerBlock + warp;
  if (query_row >= query_rows) {
    return;
  }
  ...
  const size_t q_offset = query_row * head_dim;
  const int causal_end = query_pos + 1;
  const int key_end = is_causal && causal_end < src_seq_len
                          ? causal_end
                          : src_seq_len;

  float q_cache[kValuesPerLane];
  float output_cache[kValuesPerLane];
#pragma unroll
  for (int slot = 0; slot < kValuesPerLane; ++slot) {
    const int dim = lane + slot * kWarpSize;
    q_cache[slot] = dim < head_dim ? toFloat(q[q_offset + dim]) : 0.0F;
    output_cache[slot] = 0.0F;
  }
...
#pragma unroll
    for (int slot = 0; slot < kValuesPerLane; ++slot) {
      const int dim = lane + slot * kWarpSize;
      if (dim < head_dim) {
        dot = fmaf(q_cache[slot], toFloat(k[kv_offset + dim]), dot);
      }
    }
    dot = warpSum32(dot);
...
#pragma unroll
    for (int slot = 0; slot < kValuesPerLane; ++slot) {
      ...
      if (dim < head_dim) {
        output_cache[slot] =
            fmaf(probability, toFloat(v[kv_offset + dim]),
                 output_cache[slot] * previous_scale);
      }
    }
    running_max = new_max;
  }
...
#pragma unroll
  for (int slot = 0; slot < kValuesPerLane; ++slot) {
    ...
    if (dim < head_dim) {
      output[q_offset + dim] =
          fromFloat<T>(output_cache[slot] * inverse_sum);
    }
  }
}

template <int HeadDim>
__global__ void flashAttentionFloatWarpExactKernel(
    const float* __restrict__ q, const float* __restrict__ k,
    const float* __restrict__ v, float* __restrict__ output,
    size_t query_rows, int target_seq_len, int src_seq_len, int query_heads,
    int kv_heads, int runtime_head_dim, bool is_causal) {
  ...
  if (lane == 0) {
    for (int key_pos = 0; key_pos < key_end; ++key_pos) {
      ...
      float dot = 0.0F;
      for (int dim = 0; dim < head_dim; ++dim) {
        dot = fmaf(q[q_offset + dim], k[kv_offset + dim], dot);
      }
      row_max = fmaxf(row_max, dot * scale);
    }
  }
...
#pragma unroll
  ...
  const float inverse_sum = lane == 0 ? 1.0F / row_sum : 0.0F;
  for (int key_pos = 0; key_pos < key_end; ++key_pos) {
    const size_t kv_offset =
        ((static_cast<size_t>(batch) * src_seq_len + key_pos) * kv_heads +
         kv_head) * head_dim;
    ...
    if (lane == 0) {
      ...
      probability = expf(dot * scale - row_max) * inverse_sum;
    }
    probability = __shfl_sync(0xffffffffU, probability, 0, kWarpSize);
#pragma unroll
    for (int slot = 0; slot < kValuesPerLane; ++slot) {
      const int dim = lane + slot * kWarpSize;
      if (dim < head_dim) {
        output_cache[slot] =
            fmaf(probability, v[kv_offset + dim], output_cache[slot]);
      }
    }
  }
...
#pragma unroll
  for (int slot = 0; slot < kValuesPerLane; ++slot) {
    const int dim = lane + slot * kWarpSize;
    if (dim < head_dim) {
      output[q_offset + dim] = output_cache[slot];
    }
  }
}

template <int HeadDim>
void launchFlashFloatWarpExactSpecialization(
    const float* q, const float* k, const float* v, float* output,
    size_t query_rows, int target_seq_len, int src_seq_len, int query_heads,
    int kv_heads, int runtime_head_dim, bool is_causal) {
  constexpr int kWarpsPerBlock = kAttentionBlockSize / 32;
  const unsigned int blocks = static_cast<unsigned int>(
      (query_rows + kWarpsPerBlock - 1) / kWarpsPerBlock);
  flashAttentionFloatWarpExactKernel<HeadDim><<<blocks, kAttentionBlockSize>>>(
      q, k, v, output, query_rows, target_seq_len, src_seq_len, query_heads,
      kv_heads, runtime_head_dim, is_causal);
}

void launchFlashAttentionFloatWarpExact(
    const float* q, const float* k, const float* v, float* output,
    size_t query_rows, int target_seq_len, int src_seq_len, int query_heads,
    int kv_heads, int head_dim, bool is_causal) {
#define LAUNCH_FLOAT_WARP_CASE(DIMENSION)                                     \
  case DIMENSION:                                                             \
    launchFlashFloatWarpExactSpecialization<DIMENSION>(                       \
        q, k, v, output, query_rows, target_seq_len, src_seq_len, query_heads, \
        kv_heads, head_dim, is_causal);                                       \
    break
  switch (head_dim) {
    LAUNCH_FLOAT_WARP_CASE(64);
    default:
      launchFlashFloatWarpExactSpecialization<0>(
          q, k, v, output, query_rows, target_seq_len, src_seq_len,
          query_heads, kv_heads, head_dim, is_causal);
      break;
  }
#undef LAUNCH_FLOAT_WARP_CASE
}

template <typename T, int HeadDim>
void launchFlashWarpSpecialization(
  ...
  const unsigned int blocks = static_cast<unsigned int>(
      (query_rows + kWarpsPerBlock - 1) / kWarpsPerBlock);
  flashAttentionWarpOnlineKernel<T, HeadDim><<<blocks, kAttentionBlockSize>>>(
      q, k, v, output, query_rows, target_seq_len, src_seq_len, query_heads,
      kv_heads, runtime_head_dim, is_causal);
}

template <typename T>
void launchFlashAttentionWarp(
    const T* q, const T* k, const T* v, T* output, size_t query_rows,
    int target_seq_len, int src_seq_len, int query_heads, int kv_heads,
    int head_dim, bool is_causal) {
#define LAUNCH_WARP_CASE(DIMENSION)                                          \
  case DIMENSION:                                                            \
    launchFlashWarpSpecialization<T, DIMENSION>(                             \
        q, k, v, output, query_rows, target_seq_len, src_seq_len, query_heads,\
        kv_heads, head_dim, is_causal);                                      \
    break
  switch (head_dim) {
    ...
    LAUNCH_WARP_CASE(64);
    default:
      launchFlashWarpSpecialization<T, 0>(
          q, k, v, output, query_rows, target_seq_len, src_seq_len,
          query_heads, kv_heads, head_dim, is_causal);
      break;
  }
#undef LAUNCH_WARP_CASE
}
...
void launchFlashAttention(const float* q, const float* k, const float* v,
                          ...
                          bool is_causal) {
  const size_t work_per_query =
      static_cast<size_t>(src_seq_len) * static_cast<size_t>(head_dim);
  if (head_dim >= 64 && work_per_query >= 4096 &&
      currentDeviceUses32LaneWarps()) {
    launchFlashAttentionFloatWarpExact(
        q, k, v, output, query_rows, target_seq_len, src_seq_len, query_heads,
        kv_heads, head_dim, is_causal);
    return;
  }
...
void launchFlashAttention(const half* q, const half* k, const half* v,
  ...
  if (head_dim >= 64 && work_per_query >= 4096 &&
      currentDeviceUses32LaneWarps()) {
    launchFlashAttentionWarp(q, k, v, output, query_rows, target_seq_len,
...
void rmsNorm(const std::vector<T>& h_input, const std::vector<T>& h_weight,
  ...
  RUNTIME_CHECK(
      musaMalloc(reinterpret_cast<void**>(&storage), allocation_bytes));
  ...
  RUNTIME_CHECK(musaFree(storage));
}
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
  ...
  if (src_seq_len == 0) {
    std::memset(h_o.data(), 0, query_count * sizeof(T));
    return;
  }
  ...
  RUNTIME_CHECK(
      musaMemcpy(d_q, h_q.data(), query_bytes, musaMemcpyHostToDevice));
  RUNTIME_CHECK(
      musaMemcpy(d_k, h_k.data(), kv_bytes, musaMemcpyHostToDevice));
  RUNTIME_CHECK(
      musaMemcpy(d_v, h_v.data(), kv_bytes, musaMemcpyHostToDevice));
  launchFlashAttention(d_q, d_k, d_v, d_o, query_rows, target_seq_len,
                       src_seq_len, query_heads, kv_heads, head_dim,
                       is_causal);
  RUNTIME_CHECK(musaGetLastError());
  RUNTIME_CHECK(
      musaMemcpy(h_o.data(), d_o, query_bytes, musaMemcpyDeviceToHost));
  RUNTIME_CHECK(musaFree(storage));
}
